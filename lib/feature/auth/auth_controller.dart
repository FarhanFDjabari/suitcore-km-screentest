...
import 'package:suitcore_screen_test/data/local/storage/storage_constants.dart';
import 'package:suitcore_screen_test/data/local/storage/storage_manager.dart';
import 'package:suitcore_screen_test/model/user.dart';
import 'package:suitcore_screen_test/routes/page_names.dart';
import 'package:suitcore_screen_test/utills/helper/secure_storage_manager.dart';
import 'package:suitcore_screen_test/utills/helper/shared_preference_manager.dart';
...
class AuthController extends GetxController {
  ...
  authChanged(AuthState? state) async {
    if (state?.appStatus == AppType.INITIAL) {
      ...
      clearData();
      Get.offAllNamed(PageName.FIRST);
    } else if (state?.appStatus == AppType.AUTHENTICATED) {
      Get.offAllNamed(PageName.SECOND, arguments: user!.toJson());
    } else {
